...
public static class ModSettingAPI {
    ...
    public static bool Init(ModInfo modInfo) {
        ...
        if (modInfo.name == MOD_NAME) {
            ModBehaviour.Logger.LogError($"初始化失败，不能使用ModSetting的info进行初始化");
            return false;
        }
        ...
        foreach (string methodName in methodNames) {
            ...
            if (methodInfos.Length == 0) {
                ModBehaviour.Logger.LogError($"{methodName}方法找不到");
                return false;
            }
        }
    ...
    private static bool VersionAvailable() {
        ...
        if (versionField != null && versionField.FieldType == typeof(float)) {
            ...
            if (modSettingVersion!=VERSION) {
                ModBehaviour.Logger.LogWarning($"警告:ModSetting的版本:{modSettingVersion} (API的版本:{VERSION}),新功能将无法使用");
                return false;
            }
            return true;
        }
    ...
    private static Type? FindTypeInAssemblies(string typeName) {
        ...
        foreach (Assembly assembly in assemblies) {
            if (assembly.FullName.Contains(MOD_NAME)) {
                ModBehaviour.Logger.Log($"找到{MOD_NAME}相关程序集: {assembly.FullName}");
            }
        ...
        ModBehaviour.Logger.Log($"找不到程序集");
        return null;
    }
    ...
    private static bool InvokeMethod(string cacheKey, string methodName, object?[] parameters, Type delegateType,
        ...
        try {
            ...
            ModBehaviour.Logger.LogError($"委托调用{methodName}失败: {ex.Message}");
            return false;
        }
    }
}